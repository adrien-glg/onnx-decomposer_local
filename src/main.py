...
if __name__ == '__main__':
    ...
    parser.add_argument('mode', choices=['exec', 'max_slice_size', 'payloads_per_layer', 'payloads_per_slice'],
                        help='Choose one of the available modes: exec, max_slice_size, payloads_per_layer, or '
                             'payloads_per_slice')
    mode = parser.parse_args().mode
    if mode == "exec":
        print("MODE: EXEC (DECOMPOSITION AND INFERENCE)\n")
        run(mode)
